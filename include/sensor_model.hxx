...
namespace rsim
{
    namespace smodel
    {
        ...
        struct SensorDetection
        {
            ...
            {
                for (int i = 0; i < SENSOR_COUNT; i++) detection[i] = detection_[i];
            }
        };
        ...
        public:
            ...
            {
                for (int i = 0; i < SENSOR_COUNT; i++) detection[i] = true;
            }
            ...
            SensorDetection detect(bool (&map)[cols][rows], int offset = 0)
            {
                ...
                for (unsigned long current_idx = 0; current_idx < SENSOR_COUNT; current_idx++)
                {
                    ...
                    {
                        if (cluster_start == SENSOR_COUNT + 1) { cluster_start = current_idx; }
            ...
            bool (&get_detection())[SENSOR_COUNT] { return detection; }

            void set_detection(unsigned long i, bool value) { detection[i] = value; }
